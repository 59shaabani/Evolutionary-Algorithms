...
def createCountries(config):
    ...
    r = config['number_of_choices']
    n = config['number_of_items']
    countries = []
    i = 0
    while i < k:
...
def interEmpireWar(empires, config):
    ...
    if weakest_empire.getNumberOfColonies() > 0:
        weakest_empire_colonies_cost = np.array(
            [weakest_empire.getColony(i).getCost() for i in range(weakest_empire.getNumberOfColonies())])
